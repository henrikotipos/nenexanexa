// discord.js
const { EmbedBuilder, ActionRowBuilder, ButtonBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, ModalBuilder, TextInputBuilder, PermissionFlagsBits, AttachmentBuilder } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (interaction.isStringSelectMenu()) {
            ...
            const product = await dbProducts.get(interaction.values[0]);
            if (!product) return;
            if (product) {
                ...
                const titleP = await dbPanels.get(`${panelId}.embed.title`);
                const descriptionP = await dbPanels.get(`${panelId}.embed.description`);
                const colorP = await dbPanels.get(`${panelId}.embed.color`);
                const bannerP = await dbPanels.get(`${panelId}.embed.bannerUrl`);
                const thumbP = await dbPanels.get(`${panelId}.embed.thumbUrl`);
                const footerP = await dbPanels.get(`${panelId}.embed.footer`);

                // variables with panel select menu information
                const placeholderP = await dbPanels.get(`${panelId}.selectMenu.placeholder`);
                ...
                await Promise.all(
                    allPanels.map(async (panel) => {
                        ...
                        for (const pId of productIds) {
                            ...
                            const nameP = await dbProducts.get(`${pId.replace(`p-`, ``)}.name`);
                            const priceP = await dbProducts.get(`${pId.replace(`p-`, ``)}.price`);
                            const estoqueP = await dbProducts.get(`${pId.replace(`p-`, ``)}.stock`);

                            // variables with product information by dbPanels (wio.db)
                            const emojiP = await dbPanels.get(`${panelId}.products.${pId.replace(`p-`, ``)}.emoji`);
                ...
                if (totalProducts < 1) {
                    ...
                    await interaction.reply({
                        content: `❌ | Este painel não possui produtos. Por favor, adicione produtos e atualize o painel utilizando **/config-painel**.`,
                        ephemeral: true
                    });
                ...
                await interaction.message.edit({
                                                                                        ...
                                                                                        .then(async (map) => {
                                                                                            ...
                                                                                            await interaction.user.send({
                                                                                                embeds: [new EmbedBuilder()
                                                                                                    ...
                                                                                                    .setColor(`NotQuiteBlack`)
                                                                                                    .setFooter({ text: `${client.user.username} - All rights reserved ©` })
                                                                                                ]
                                                                                            }).then(async (msg) => {
                    ...
                    if (err.code == 50035) {
                        await interaction.reply({
                            content: `❌ | Erro no select menu! Por favor, verifique se os emojis dos produtos estão configurados corretamente utilizando **/config-painel**.`,
                            ephemeral: true
                        });
                    };
