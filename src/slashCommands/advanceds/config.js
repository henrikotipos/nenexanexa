...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const embedProduct = new EmbedBuilder()
            .setTitle(`CONFIGURANDO PRODUTO`)
            .setDescription(`**📜 | Descrição:\n\`\`\`${descriptionP}\`\`\`\n📝 | ID: \`${idProduct}\`\n🛒 | Produto: \`${nameP}\`\n💸 | Preço: \`R$${Number(priceP).toFixed(2)}\`\n📦 | Estoque: \`${estoqueP.length}\`**`)
            .setColor(`NotQuiteBlack`)
            .setFooter({ text: `${client.user.username} - All rights reserved ©`, iconURL: client.user.avatarURL() });
